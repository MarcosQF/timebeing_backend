...
def upgrade() -> None:
    ...
    op.execute("ALTER TABLE task ALTER COLUMN notify_at TYPE INTERVAL USING notify_at * INTERVAL '1 second'")

def downgrade() -> None:
    """Downgrade schema."""
    # Converte interval para integer (segundos)
    op.execute("ALTER TABLE task ALTER COLUMN notify_at TYPE INTEGER USING EXTRACT(EPOCH FROM notify_at)::integer")
